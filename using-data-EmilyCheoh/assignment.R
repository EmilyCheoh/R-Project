...
write.csv(students, "StudentData.csv", row.names=FALSE)
...
titanic.data <- data.frame(Titanic, stringsAsFactors = FALSE)
...
lost <- titanic.data[titanic.data$Survived=='No',]
most.lost <- lost[lost$Freq == max(lost$Freq),]
...
SurvivalRate <- function(ticket.class) {
  class.stats <- titanic.data[titanic.data$Class == ticket.class,]
  women.and.kids <- class.stats[(class.stats$Sex == "Female") | (class.stats$Age == "Child"),] # get women and kids' info
  num.women.and.kids <- sum(women.and.kids$Freq)
  survived.women.and.kids <- sum(women.and.kids$Freq[women.and.kids$Survived == "Yes"]) # how many women and kids had survived
  women.and.kids.rate <- paste(round(survived.women.and.kids / num.women.and.kids * 100, 0), "%", sep = "")
  men <- class.stats[(class.stats$Sex == "Male") & (class.stats$Age != "Child"),] # get men's info
  num.men <- sum(men$Freq)
  survived.men <- sum(men$Freq[men$Survived == "Yes"])
  men.rate = paste(round(survived.men / num.men * 100, 0), "%", sep = "")
  return(paste("Of", ticket.class, "class,", women.and.kids.rate, "of women and children survived and", men.rate, "of men survived."))
}

# Call your `SurvivalRate()` function on each of the ticketing classes (`Crew`, `1st`, `2nd`, and `3rd`)

crew.rate <- SurvivalRate("Crew")
first.rate <- SurvivalRate("1st")
second.rate <- SurvivalRate("2nd")
third.rate <- SurvivalRate("3rd")
...
num.small.gain <- length(which(life.expectancy$change < 5))
...
LowestLifeExpInRegion <- function(region.name) {
  this.region <- life.expectancy[life.expectancy$region == region.name,]
  return(this.region$country[this.region$le_2013 == min(this.region$le_2013)])
}
...
BiggerChange <- function(country1, country2) {
  c1.change <- CountryChange(country1)
  c2.change <- CountryChange(country2)
  if (c1.change > c2.change) {
    diff <- c1.change - c2.change
    bigger <- paste(country1, " (gain=", round(c1.change, 1), "),", sep = "")
    smaller <- paste(country2, "'s (gain=", round(c2.change, 1), ").", sep = "")
  } else {
    diff <- c2.change - c1.change
    bigger <- paste(country2, " (gain=", round(c2.change, 1), "),", sep = "")
    smaller <- paste(country1, "'s (gain=", round(c1.change, 1), ").", sep = "")
  }
  return(paste("The country with the bigger change in life expectancy was", bigger, "whose life expectancy grew by", round(diff, 1), "years more than", smaller))
}
...
highest.average.change <- HighestAverageChange()

HighestAverageChange <- function() {
  regions <- unique(life.expectancy$region)
  max.gain <- -100
  max.region <- ""
  for (i in life.expectancy$region) {
    num.country <- length(unique(life.expectancy$country[life.expectancy$region == i]))
    gain.of.this.region <- sum(life.expectancy$change[life.expectancy$region == i])
    average.gain <- gain.of.this.region / num.country
    if (average.gain > max.gain) {
      max.gain <- average.gain
      max.region <- i
    }
  }
  return(max.region)
